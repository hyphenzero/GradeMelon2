...
import { findCurrentPeriod, getCache, Grades, SchoolsListType } from '../utils/grades'
import { reducedMotionConfig, springConfig } from '../utils/motionConfig'
import { attendance, document, grades as sample, schedule, studentInfo as info } from '../utils/sample'
...
function MyApp({ Component, pageProps }) {
  ...
  function guestLogin() {
    setClient({
      ...
      loadedDocuments: [
        {
          file: { date: new Date(), type: 'Sample' },
          comment: 'Sample Document',
          get: () => {
            return [{ base64: document }]
          },
        },
      ],
    } as any)
...
*/
    ...
    await StudentVue.login(
      ...
      .then(async (res) => {
        ...
        extraData.gradingScale.mode =
          fetchedClient.district == 'https://md-mcps-psv.edupoint.com/Service/PXPCommunication.asmx'
            ? 'mcps'
            : undefined
...
*/
  ...
  async function getAd() {
    ...
    const response = await fetch(
      adServer + '/serve?school=' + encodeURIComponent(schoolName) + '&' + 'grade=' + encodeURIComponent(grade),
      {
        method: 'GET',
      }
    )
  ...
  async function buildConcurrentCache(gu): Promise<SchoolsListType> {
    ...
    const remainder = await Promise.all(
      initalFetch[0].reportingPeriod.available.map((period) => {
        if (period.index == initalFetch[0].reportingPeriod.current.index) {
          return initalFetch
        } else {
          return client.gradebook(period.index, gu)
        }
      })
    )
  ...
  useEffect(() => {
    if (studentInfo?.schools?.length > 0 && grades && !schoolsList) {
      const schoolsData = Promise.all(
        studentInfo.schools.map(async (school) => ({ ...(await buildConcurrentCache(school.GU)), name: school.name }))
      )
      schoolsData.then((data) =>
        setSchoolsList([{ name: studentInfo.currentSchool, cache: grades, mp: mp, gu: null }, ...data])
      )
    }
  ...
  useEffect(() => {
    //Hook responsible for fetching studentInfo
    if (client !== undefined && studentInfo == undefined) {
      if (localStorage.getItem('infoCache') != undefined) {
        //temporarily re-enabling infoCache
      ...
      client
        ...
        .catch((error) => {
          console.log(error, 'fuck me sideways and backwards')
          client
            .studentInfo()
            .then(([info]) => {
              setStudentInfo(info)
  ...
  useEffect(() => {
    ...
    if (
      ...
      districtURL !== undefined
    ) {
      doLogin()
    } else {
      if (
        client === undefined &&
        (!noShowNav.includes(router.pathname) || router.pathname == '/') &&
        !refURL &&
        !guest
      ) {
  ...
  return (
    <>
      ...
      <Head>
        ...
        <script
          async
          src="https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-pub-3837623952720969"
        />
        <script
      ...
      <div className="fixed z-[60] p-5">
        {toasts.map(({ title, type }, i) => (
          <div className="z-50 mb-5" key={i}>
      ...
      <div className="min-h-screen bg-zinc-50 pt-16 dark:bg-zinc-950">
          ...
          {client && !isMediumOrLarger && (
            <div className="pb-16 md:pb-0">
              <div className="md:hidden">
                ...
                {modalBg && <div style={{ opacity: 0.1 }} className="fixed inset-0 z-0 bg-gray-500"></div>}
              </div>
            </div>
          )}
        </div>
